...
int main()
{
    ...
    // * if (this == &v) используется для предотвращения процесса самоприсваивания.
    Vector a(2, 1), b;
    ...
    // Вызывается ли эта операция в этом фрагменте? Почему?
    MyString ss = s;
    // * Объект ss создается и сразу инициализируется значением s => вызывается конструктор копирования
    ...
    // по правилам сложения векторов на плоскости
    ...
    /*
     * Результатом скалярного произведения является число => можно реализовать просто как метод класса
     * с возвращаемым значением типа double, т.к. левый операнд всегда будет объект класса.
     */
    ...
    // класса и нажмите OK. Студия введет в состав проекта два новых файла.
    ...
    // 2. С 2-мя параметрами (приращение вширь и приращение вверх-вниз).
    // 3. С 4-мя параметрами (приращения, различные для всех 4-х границ).
